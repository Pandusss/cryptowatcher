...
Price provider from MEXC WebSocket (via Redis cache).
...
class MEXCPriceAdapter(BasePriceAdapter):
    async def get_price(self, coin_id: str) -> Optional[Dict]:
        return await self._get_price_from_redis(coin_id, "mexc", "MEXCPriceAdapter")
    ...
    def is_available(self, coin_id: str) -> bool:
        return coin_registry.find_coin_by_external_id("mexc", coin_id) is not None

# Global instance
mexc_price_adapter = MEXCPriceAdapter()
